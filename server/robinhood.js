...
function RobinhoodWebApi(opts, callback) {
  ...
  var _options = opts || {},
    ...
    _endpoints = {
      ...
      options_historical: 'marketdata/options/historicals/',
    ...
    _request.post(
      {
        ...
        form: {
          ...
          device_token: 'd3980c97-e39e-4b03-a09b-3dc1a6f52a00',
          // expires_in: 86400,
  ...
  function get_options_history (instrument_ids) {
    return new Promise((resolve, reject) => {
      let request = _request.get(
        {
          uri:
            _apiUrl +
            _endpoints.options_historical +
            `?bounds=regular&start=2020-01-01T00:00:00Z&span=week&interval=10minute&instruments=` +
            instrument_ids.join('%2C') +
            `&ratios=${instrument_ids.map(o => `1`).join('%2C')}` +
            `&types=${instrument_ids.map(o => `long`).join('%2C')}`
        },
    ...
    return new Promise((resolve, reject) => {
      ...
      return _request.get(
        ...
        (err, res, body) => {
          ...
          if (body.results.length === 1) return resolve(body.results[0]);
  ...
  api.options_dates = function (symbol, callback) {
    return new Promise((resolve, reject) => {
      api.instruments(symbol, function (err, response, { results }) {
        if (err) throw err;
        let { tradable_chain_id } = results.find(o => o.symbol === symbol);
        _request.get(
          {
            uri:
          ...
          function (err, response, { expiration_dates }) {
            if (err) reject(err);
            resolve({ tradable_chain_id, expiration_dates: response.body })
          }
        );
      });
    })
  ...
  api.options_available = function (
    ...
    return new Promise((resolve, reject) => {
      if (type !== "call" && type !== "put") return console.warn(`"type" must be "call" or "put".`);
      let request = _request.get(
        {
  ...
  api.option_history = function (chain_id) {
    return get_options_history(chain_id)
  }
